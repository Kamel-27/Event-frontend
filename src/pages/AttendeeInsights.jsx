...
import { getAllEvents, getAttendeeInsights } from "../api";
import { 
  Search, 
  Users, 
  MapPin, 
...
const AttendeeInsights = () => {
  ...
  const fetchEvents = async () => {
    try {
      setLoading(true);
      const eventsResponse = await getAllEvents({ status: 'active', limit: 100 });
      ...
      if (eventsResponse.data.success) {
        ...
        if (eventsResponse.data.events.length > 0) {
          ...
          fetchEventInsights(eventsResponse.data.events[0]._id);
        }
      }
    } catch (error) {
      ...
      <div className="p-6">
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-800 mb-4">
            Attendee Insights - {selectedEvent?.name || "Select Event"}
          </h1>
          ...
          <div className="mb-4">
            <select
              ...
              className="px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#0df38a]"
            >
        ...
        {!selectedEvent ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">Choose an event from the dropdown above to view attendee insights</p>
          </div>
        ) : (
          <>
            {/* Key Metrics Cards */}
                          ...
                          return (
                            <path
                              ...
                              fill={item.color.replace('bg-', '').includes('blue') ? '#3B82F6' : 
                                    ...
                                    item.color.replace('bg-', '').includes('red') ? '#EF4444' :
                                    item.color.replace('bg-', '').includes('purple') ? '#8B5CF6' : '#EAB308'}
                            />
                          );
                        })}
                      </svg>
